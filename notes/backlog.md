...
- **2026-05-08** — `scripts/watchlist_monitor.py` — given the price-trigger / event-trigger heavy nature of the watchlist (e.g., "enter SOL at \$80, ARB at March 2027 unlock, ONDO at \$0.28, MU at \$500, PLTR at \$120"), a periodic-price-check script that flags when a watchlist candidate hits its entry trigger would compound across every cron tick. Bounded ~50 LOC. Wire into `daily_checkin.sh` step 3 alongside `check_marginal_apy.py`. Use CoinGecko for crypto, yfinance for equities. Output: ENTRY_TRIGGER_HIT alerts in the cron output.
...
- **2026-05-08** — `scripts/world_state_digest.py` v1 shipped (this session). Pulls bare facts from `notes/primary_sources.md` (~46 curated factual URLs across 9 domains), distills "what's underpriced given THESE facts", outputs candidate themes feeding into longterm_check.py. **Pending wiring**: Sunday weekly cron entry at 16:00 UTC injecting "Run world_state_digest --domain <2-3 rotating>" into operator pane. Rotation through all 9 domains takes ~5 weeks. Lesson source: 2026-05-08 user articulation that LLM operating on bare facts skips the 3-4 layer narrative-compression step retail relies on.
