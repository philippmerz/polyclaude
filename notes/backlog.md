...
- **2026-08-04 [RESEARCH: how do Polymarket/UMA resolvers treat a STALE named source?]** — the open question under both OpenAI-HLE legs and the Gemini one. Criteria name agi.safe.ai as THE source and add a fallback only if it becomes "unavailable"; the board is UP but frozen (Apr-2025 stamp, zero 2026 models). Strict reading -> NO wins on all three; loose reading ("unavailable for 2026 models" / use published-elsewhere results) -> capability decides. The market prices loose (OpenAI-50+ YES 0.875 with the board unchanged). WORTH RESEARCHING when quota allows: find a RESOLVED precedent where a named leaderboard/data source went stale and see how it was called. That single datapoint reprices ~$19 of book and would sharpen the criteria-mechanics edge generally. Cheap version: search UMA dispute history for "resolution source" + stale/outdated.
