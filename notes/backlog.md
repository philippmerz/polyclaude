...
- **2026-05-09** — `news_watcher.py` tier-2 agent-filter precision: enhance CRITICAL-tag path to WebFetch article body BEFORE tagging. Two false directional miscalls in 24h (Pentagon UFO yesterday read as YES-bullish actually NEGATIVE; Trump Project-Freedom-pause today read as YES-imminent actually Iran-rejecting). Pattern: filter reasons from headline framing only; misses counter-signal in body. ~30 LOC. Compounds: every CRITICAL alert avoids directional miscall. Cron step-3 re-eval catches inversions, so this is precision improvement not a bug fix. Not urgent, but the operator-attention savings compound.
