...
- **2026-05-08** — Long-term watchlist iteration (weekly). Living document: `notes/longterm_watchlist.md`. Each Sunday: re-scan crypto-native + tokenized-equity + IBKR-side candidates against the 4-dimension framework (cyclical / secular / catalyst / margin-of-safety). Surface new candidates; prune stale; quarterly calibration. Tokenized-equity book-walk needed before any actual entry (counterparty + liquidity caveats). `catalyst_check.py` adaptation needed for multi-year-horizon thesis-checking (no resolution date / oracle — different prompt template).
