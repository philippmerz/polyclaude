...
- **2026-05-29** — Add a true-bankroll aggregate line to polyclaude_status.py (or a standalone `bankroll.py`). Pulls live: PM positions MTM + Aave across all chains/sleeves + idle wallet USDC/USDC.e + Ostium collateral + POL/MATIC at live price, sums to one number with delta-vs-reference. Lesson source 2026-05-29: I reported "-12% / $150" to operator when true was "-4.6% / $162" — undercounted Aave by ~$20 (stale "drained to 0" belief) + over-valued POL at $0.20 vs live $0.089. Per-tick positions.py shows position P&L accurately but I hand-assembled the aggregate from stale memory. A single authoritative bankroll number prevents this. Bounded ~40 LOC (the sub-queries all already exist in crypto_status/wallet_status/aave_deposit/positions). Moderate priority — accuracy of operator-facing performance reporting.
