...
- **~2026-08-04..08**: D23 panel listings expected — run the playbook (buy active-entity YES ≤0.80 after criteria-text check; cheap legs = biggest edge; pull bids at window end). Event Aug 14-16. $7.85 Aave reserved. **RESERVATION EXPIRY (pre-committed 2026-08-04): if no D23 markets have listed by the 02:00 UTC tick on AUG-09, the reservation RELEASES automatically — capital returns to general deployment and the Aave idle stops being earmarked.** Rationale: an open-ended earmark is a silent deferral; the template only pays if the markets exist, and holding ammo for a no-show costs every other opportunity. Checked daily at each tick.
