...
## Current state (snapshot 2026-09-08 14:01 UTC; separate checks timestamped below)
...
**Whole-account performance:** indicative net liquidation **~$172.51**, including **$6.22 gas-token value**; excluding gas, **~$166.29 versus $170 contributed: −$3.71 (−2.18%) before VM/other operating costs**. Authoritative marked bankroll is **$180.24 at 14:01:55 UTC**, down $1.87 from the 11:00 operator report. PM midpoint **$146.78** versus indicative fee/depth net **$139.05** gives a $7.73 gap. Net liquidation is derived by replacing the PM midpoint component of the authoritative total, using rounded outputs; the sequential books are not synchronized or freshness-verified executable quotes, and transfer/withdrawal costs are excluded. Settled P&L **+$14.44** is secondary: unrealized liquidation losses **−$18.15** outweigh it (marked unrealized **−$10.42**). Thirteen active PM rows cost $157.20. The authenticated 14:01 check still has five LIVE maker SELLs, each with `size_matched=0`, zero BUYs and zero Apple orders; this is not a claim about all historical fills. No trade/order change this tick. Earlier deterioration evidence: [10:00 depth review](research/2026-09-08-1000-portfolio-depth-check.md).

**Holds and gates:** Gemini ≥50 / debut ≥40 priors remain **0.55 / 0.60**; OpenAI ≥50 / ≥55 **0.35 / 0.40**; Apple **0.65**; Trump-out / Greenland **0.97 / 0.95**. Apple NO is **0.145** on Gamma. Main's **14:02:50** exact-market book (.12/.17 touch) estimates **$5.45 net** for 49.005 shares, but its timestamp is **755 seconds old**: not a fresh executable quote. The correct-wallet 14:03:05 inventory independently confirms the held token and quantity. The bounded source check found no firm new purchase date; the decline remains unexplained, not dismissed as quote noise. Stored priors and their stressed hold values remain unproven estimates. Duma retains 20 equal shares in each covered bucket, union fair **0.68**; no add because the evidence gate requires ≥0.75. Exit diagnostics value the whole Duma/MetaMask groups at about **$12.00/$44.57–44.69** versus $13.60/$46.80 raw fair, across separate sequential reads. MetaMask's rule-based minimum payout is $44.751 before time/resolver risk, not a stop price. All ordinary/group exit verdicts are HOLD; no group-leg changes or new capital deployment. The 14:01 HLE check found the same ten rows and both score columns as Jan-15, with expected-family coverage passing. UMA retrieval succeeded and flagged Apple's +9pp YES move; no new finality alert was reported. Interpretation/UMA risk, correlated-cluster caps and no-add gates remain. See the [backlog](notes/backlog.md) and [calibration cohort audit](research/2026-09-08-calibration-cohort-audit.md).
...
**Depth/quantity verification, 14:05:55–14:06:00 UTC:** all 13 active quantities match the pre-tick committed claims within 1e-6; all Gamma condition/token identities match. Thirteen complete walks, zero empty books/unfilled shares, net **$138.89769104**, including Duma **$11.78** and MetaMask **$44.45612211**. Four books exceed 60 seconds (Apple 944s); this later sequential check neither replaces the authoritative 14:01 snapshot nor establishes executable portfolio proceeds.
