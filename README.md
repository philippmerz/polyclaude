...
## Current state (snapshot 2026-09-01 22:03 UTC)
...
**Total bankroll $208.93; REALIZED +$22.04 (+13.0%) vs the $170 reference** — realized settled cash is the metric that counts. The **13 active indexed PM rows** cost $157.20 and mark at $168.22 (+$11.03), but executable-depth value is $162.65 (+$5.46): quote both because thin books overstate the active sleeve by $5.57. Data-api also retains one settled losing Iran–Oman row, now excluded from open P&L and advisory tools. It finalized NO on Sep-1; DEC-0082's 31 YES shares lost **$5.029998 gross**, correcting realized P&L immediately instead of waiting for de-indexing. `redeem-all` correctly sent no transaction: it now requires the held outcome itself to be final at $1, because data-api labels losing rows redeemable too. Hormuz-normal is de-indexed and still unproposed, but DEC-0086 had already sold 12.11 NO; the archive correctly retains only **0.003571 dust**, not a live $12 claim. Six maker sells and zero BUY commitments rest. Duma remains a 0.68-fair equal-share HOLD with its add gate failed. The wallet has 9.732500 deployable pUSD and 16.103258 Polygon aUSDC.e. GTA's 22:01 counter burst still failed the fee-aware robust re-entry gate. The restarted `news_watcher` remains singular on current code; its only fresh Iran story correctly mapped as a MINOR Trump-out risk rather than a settled-position alert. Money-math suite: 123 checks plus a 19-case mutation harness, pre-commit enforced. Short-dated ledger N=51.
