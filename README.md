...
## Current state (snapshot 2026-09-01 14:25 UTC)
...
**Total bankroll $206.97; REALIZED +$22.04 (+13.0%) vs the $170 reference** — realized settled cash is the metric that counts. The **13 active indexed PM rows** cost $157.20 and mark at $166.13 (+$8.94), but executable-depth value is $159.37 (+$2.18): quote both because thin books overstate the active sleeve by $6.76. Data-api also retains one settled losing Iran–Oman row, now excluded from open P&L and advisory tools. It finalized NO on Sep-1; DEC-0082's 31 YES shares lost **$5.029998 gross**, correcting realized P&L immediately instead of waiting for de-indexing. `redeem-all` correctly sent no transaction: it now requires the held outcome itself to be final at $1, because data-api labels losing rows redeemable too. Hormuz-normal is de-indexed and still unproposed, but DEC-0086 had already sold 12.11 NO; the archive correctly retains only **0.003571 dust**, not a live $12 claim. Six maker sells and zero BUY commitments rest. Duma remains a 0.68-fair equal-share HOLD with its add gate failed. The wallet has 9.732500 deployable pUSD and 16.102832 Polygon aUSDC.e. Sports consensus now rejects missing, stale, or date-conflicting bookmaker evidence; the current Álvarez/Atlético candidate did not clear the robust gate at a 0.9875 market price. Money-math suite: 123 checks plus a 19-case mutation harness, pre-commit enforced. Short-dated ledger N=51.
...
- `decisions.json` — append-only structured decision tracker (DEC-0001 through DEC-0112 as of this snapshot)
