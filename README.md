...
**Last updated:** 2026-08-31 (state snapshot updated every cron tick; this header on structural changes)
...
## Current state (snapshot 2026-08-31 14:15 UTC)
...
**Total bankroll $212.70; REALIZED +$33.59 (+19.8%) vs the $170 reference** — realized settled cash is the metric that counts (operator directive 2026-08-18: "it's only truly return when the cash settles on the wallet"). The 14 PM rows cost $162.23 and mark at $165.48 (+$3.25), but executable-depth value is $159.49 (-$2.73): quote both because thin books overstate the open sleeve by $5.98. Oman's official statement index still has no bilateral acceptance announcement. Six post-10:00 alert entries only corroborated the already-processed US–Iran exchange; the Iran–Oman YES prior remains 0.04 and its 31 shares remain HOLD/no-add because $1.24 expected terminal value exceeds the roughly $0.31 executable exit through the Sep-1 03:59 UTC cutoff. Six maker sells and zero BUY commitments rest. The Aug-31 HLE review validated the archive instrument against known 2025 changes, passed Claude/Grok/Gemini coverage, and again found the live agi.safe.ai board identical to 2026-01-15. The next-Gemini-Pro debut NO ladder asks 0.15/0.17/0.31/0.62 at the 40/45/50/55 thresholds. The held ≥40 leg remains the best expression at p_no 0.60 / stress 0.45, but no add clears the full entry pipeline: the correlated `ai-ships-fast-short` cap is effectively binding and the helper rejects the sub-$1 incremental size. The ticket stays 169 NO, $17.60 notional / $18.22940 fee-inclusive. The Duma distribution was also rechecked: new Aug-31 poll coverage still supplies no independent whole-seat forecast, so the 0.76 union prior and Sep-1..10 add gate remain unchanged. The strongest thin-tail candidate, GTA VI `<20M` one-week YouTube views, was measured at 15.703M after 85.2 hours but skipped because plausible decay models span both sides of 20M and neither side clears the 10-point robustness gate; it remains on a per-tick velocity watch through Sep-4 01:01 UTC. Unused proceeds earn on 26.3512 Polygon aUSDC.e at a 2.96% live hurdle; exactly $6.000000 pUSD remains immediately deployable, inside the $5–10 operational-float band. Money-math suite: 123 checks plus a 19-case mutation harness, pre-commit enforced. Short-dated ledger N=51.
...
- `decisions.json` — append-only structured decision tracker (DEC-0001 through DEC-0106 as of this snapshot)
