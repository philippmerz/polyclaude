...
## Current state (snapshot 2026-08-10 03:24 UTC)
...
**Total bankroll $175.97 marked / ~$173 realizable, +3.5% vs $170 reference** (bankroll.py; natives temporarily unvalued on a CoinGecko timeout, so ~$4 understated — the script warns rather than silently omitting). 8 PM positions ($157.59 MTM on $147.80 cost, +6.6% marked, **+4.7% at best bids**) + 1 resting maker bid + 3 resting maker sells. Worked example of why both figures are quoted: touchscreen-MacBook NO round-tripped 0.495 → 0.685 → 0.420 in a single day, and the 0.685 was a MIDPOINT inside a 0.57/0.76 spread on ZERO 24h volume — a $7.59 phantom that positions.py and bankroll.py now refuse to count, automatically and without being asked. Dec-31 fade book (Trump-out NO, Greenland NO — both re-verified 2026-08-11: no removal majorities assembled, Greenland/Denmark still refusing sale) + instance entries (SpaceX-IPO YES, touchscreen-MacBook NO, GPT-6-by-Aug-31 NO) + 3 HLE leaderboard NO legs. **HLE crux re-verified from source 2026-08-10**, correcting a stale inference: the Apr-2025 stamp on agi.safe.ai is the DATASET date, not a frozen board — the board does accept post-stamp models but has added none since ~Nov-2025 (no GPT-5 Pro, Opus 5, Fable 5 or Gemini 3.1), while OpenAI's best score anywhere is 49.5, under the 50 bar. Criteria gate the 'published elsewhere' escape on the source being UNAVAILABLE, and default to NO. Priors are now decomposed rather than vibes (P(board posts a row) + P(no row)x w_spirit x P(capability)) and any prior differing from mark by >25pp is flagged by an automated check that only a dated, expiring acknowledgement can silence. GPT-6 NO +21% after the Astra-leak episode round-tripped; its criteria were re-read 2026-08-10 and the thesis-break rule is now explicit (open beta / open rolling waitlist = exit; closed beta or private access = hold). **Maker-first execution:** 3 resting post-only take-profit sells at/above fair plus 1 resting post-only BUY (GPT-6/MacBook/HLE excluded from sells — hidden-info class). Realized recently: Marvel-SDCC +43.9%, Prime-SDCC +59%, ARB +11.8%, Satoshi + Fed wash exits. Brier ledger N=9: 0.164 vs market 0.203 (beating; N too small for a verdict).
