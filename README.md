...
**Last updated:** 2026-09-08 (state snapshot updated every cron tick; this header on structural changes)
...
## Current state (snapshot 2026-09-08 08:49 UTC; separate depth check 08:51)
...
**Bankroll:** authoritative total **$191.22 at 08:49:23 UTC**, up $0.01 from 07:49; settled profit **+$14.44 (+8.5%)** versus the $170 reference is unchanged. Gas-token value of $6.25 is excluded from trading P&L. The **08:49:19.557694 UTC** inventory confirms 13 active PM rows costing $157.20, with unchanged quantities. PM midpoint is $157.74; the indicative fee/depth estimate is $150.26 (the full-precision warning gap rounds to $7.47), down about $1.55 from 07:49. A separate 08:51 exact-identity check covers all 13 quantities without empty or insufficient books and sums to **$150.22000852 net**, but three books exceed 60 seconds old. Neither estimate is a synchronized, freshness-verified liquidation quote, and the earlier per-leg quotes were not retained, so the historical change cannot be attributed exactly. Five live maker SELL orders remain, each with `size_matched=0` in the authenticated 08:49 check (HTTP 200); zero BUYs and zero Apple orders.

**Holds and gates:** Gemini ≥50 / debut ≥40 priors remain **0.55 / 0.60**; OpenAI ≥50 / ≥55 **0.35 / 0.40**; Apple **0.65**; Trump-out / Greenland **0.97 / 0.95**. Apple's 08:51 held-NO mark is **0.29**; the complete depth-walk estimate remains **$13.14 net**, versus $31.85 raw / $26.95 10pp-stressed hold value. Its book was **106.7 seconds old**, so this is indicative only. The bounded 07:34 source check found no new purchase-date commitment; current product absence and late-2026/early-2027 reporting do not prove NO or explain the move. Duma retains 20 shares in each of three buckets, union fair **0.68**; its add gate still fails on evidence (**0.68 < 0.75**) and the last 02:00 common-min ask (**0.687768 > 0.57**, not refreshed). The separate 08:51 Duma/MetaMask sequential-book exit estimates are **$13.18/$44.53** versus $13.60/$46.80 raw fair. MetaMask's rule-based minimum payout is $44.751 before time/resolver risk, not a stop price; its $700M book was 72.5 seconds old. No new thesis fact or group-level exit case was established, and neither group permits independent leg changes. The 10-row named HLE table and both score columns were unchanged at **07:58**; its earlier historical control passed. The **08:49** resolution-risk refresh checked 35 unique Gamma markets without alerts, with a valid position fetch. Interpretation/UMA risk, correlated-cluster caps and no-add gates remain in force. See the [backlog](notes/backlog.md) and [latest calibration cohort audit](research/2026-09-08-calibration-cohort-audit.md).
...
**Reminders and balances:** Apple’s official keynote is Sep 9 at 17:00 UTC, with the one-shot review armed for 18:30; AVAV research is scheduled for Sep 9 at 22:00 UTC per the backlog. There is no economic Hormuz exposure beyond **0.003571** dust; Iran–Oman is closed at a $5.03 loss already reflected in settled P&L. The 08:49 balances round to **$2.12 pUSD**, **$16.11 Polygon aUSDC.e**, and **$7.87 Arbitrum aUSDC**. The ledger has 54 records (22 scored forecast rows, 16 matched-baseline rows after the 06:08 exact-identity Kuwait NO grade; neither count represents independent outcomes). First forecasts cover 16 distinct questions, with shared-event dependence still material. No original forecast or price changed. Tests are **643 total / 156 money-math checks**, including 38 isolated calibration tests and three missing-quote watchlist-reporting regressions. The earlier disk-full ledger truncation was fully recovered, field-verified and journaled at 05:15 UTC. Free space is **510.25 MiB at 09:15**, below the 512 MiB warning threshold again; the normal **09:14:45** heartbeat warning was verified, with no critical/probe-error alert. Capacity remains open for the 10:00 periodic recheck. The deployed hourly probe and targeted write guard do not substitute for durable headroom or authorized root-owned cleanup.
...
- `source_freeze_check.py` — validated archive comparison. On agi.safe.ai, the default compares every results-table model and both displayed score columns, detecting new naming families and score-only edits; malformed/ambiguous tables fail closed. Other sources or non-default regexes explicitly report token-only coverage, not whole-page stasis.
