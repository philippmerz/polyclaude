...
**Last updated:** 2026-08-30 (state snapshot updated every cron tick; this header on structural changes)
...
## Current state (snapshot 2026-08-30 10:01 UTC)
...
**Total bankroll $217.21; REALIZED +$33.59 (+19.8%) vs the $170 reference** — realized settled cash is the metric that counts (operator directive 2026-08-18: "it's only truly return when the cash settles on the wallet"). The 14 PM rows cost $162.23 and mark at $169.41 (+$7.18), but executable-depth value is $160.14 (-$2.08): quote both because the thin books overstate the open sleeve by $9.26. Since 18:03, marked value fell $1.07 and executable value fell $1.35. Two material alerts reported the first US strike in Iran since late July and an IRGC retaliation threat; the Iran–Oman YES prior was cut 0.08→0.05, but its 31 shares remain HOLD/no-add because $1.55 expected terminal value still exceeds the roughly $0.68 executable exit. Six maker sells and zero BUY commitments rest. DEC-0085's 40.95563 Lake-America YES resolved early at 05:54 UTC and was redeemed for 40.955630 USDC.e; fee-inclusive profit was $28.616281 (+231.9%) on $12.339349 risk. The server-HTML Maps proxy never fired, while Google's official product post said the US update was rolling out; the proxy remains review-only and `uma_status_check` now explicitly alerts on resolved/de-indexed held markets via cached direct IDs. Lake's payout opened factor-cap room, so DEC-0102 bought exactly 70 more next-Gemini-Pro-debut ≥40 NO at raw 0.12 FAK ($8.40 plus $0.29568 fee). That ticket is now 169 NO, $17.60 notional / $18.22940 fee-inclusive. The Aug-30 archive diff again detected real 2025 board changes and found the live model-results board identical to 2026-01-15, but CAIS's official repository shows active July dataset maintenance; the prior was therefore cut 0.65→0.60 and its 15-point stress to 0.45 rather than treating scoreboard stasis as abandonment. The direct falsifier remains a qualifying board row—already-released Gemini 3.1 Pro scores 46.44 on Scale, and Google says 3.5 Pro is coming soon. Execution-time `ai-ships-fast-short` gross exposure was $63.229 versus a $65.034 cap; further adds remain barred without new headroom plus another full-cluster review. Unused proceeds now earn on 26.3497 Polygon aUSDC.e at 3.144% observed APY; exactly $6.000000 pUSD remains immediately deployable, inside the $5–10 operational-float band. The Duma set remains 20 equal YES shares across 295–309 / 310–324 / 325–339 and must never be traded leg-by-leg. Money-math suite: 123 checks plus a 19-case mutation harness, pre-commit enforced. Short-dated ledger N=51.
...
Ostium: 0 open perps (SPX / NDX / XAU all TP-closed May-2026; planned OLP deposit parked pre-exploit — zero exposure through both exploit reports, $18M→$24M revised). Crypto sleeve has ~$7.87 aUSDC Aave-Arb plus small stable/gas balances. The PM wallet holds 26.3497 aUSDC.e on Polygon, exactly $6.000000 pUSD operational float, and small raw stable/gas balances after the Lake redemption and DEC-0102 redeployment. Status scripts read aTokens/pUSD directly, so idle capital is always visible. Run `scripts/crypto_status.py` + `scripts/aave_deposit.py rate` for live figures.
...
- `decisions.json` — append-only structured decision tracker (DEC-0001 through DEC-0103 as of this snapshot)
