...
## Current state (snapshot 2026-09-08 02:02 UTC)
...
**Bankroll:** total bankroll **$192.15**, down $6.34 from the 01:04 snapshot, mainly Apple/Gemini marks; settled profit **+$14.44 (+8.5%)** versus the $170 reference is unchanged. The $6.25 gas deposits are excluded from trading P&L, not trading capital. The 13 active PM rows cost $157.20; midpoint is $158.66, while the depth/fee estimate is $151.62 (gap $7.04). That estimate is **not** a synchronized, freshness-verified liquidation quote. Five live maker SELL orders remain, all five with `size_matched=0` at the authenticated 01:52 check; there were zero BUYs, zero Apple orders, and no new fills detected.

**Holds and gates:** Gemini ≥50 / debut ≥40 priors remain **0.55 / 0.60**; OpenAI ≥50 / ≥55 **0.35 / 0.40**; Apple **0.65**; Trump-out / Greenland **0.97 / 0.95**. Apple's YES rose 5.5pp to 0.675; a fresh criteria/store/event review found no new public-purchase evidence, not proof the move is noise. Its $14.78 indicative complete exit has an over-age book timestamp, versus $31.85 raw / $26.95 10pp-stressed hold value. Duma retains 20 shares in each of three buckets, union fair **0.68**; its add gate fails independently on evidence (**0.68 < 0.75**) and observed common-min ask (**0.687768 > 0.57**). Duma/MetaMask sequential-book exit estimates are $13.15/$45.16 versus $13.60/$46.80 fair; neither group permits leg-level changes. The 10-row named HLE table and both score columns are unchanged in the 02:00 check; its historical control passed. Interpretation/UMA risk, correlated-cluster caps and no-add gates remain in force. See the [backlog](notes/backlog.md) and [latest calibration cohort audit](research/2026-09-08-calibration-cohort-audit.md).
...
**Reminders and balances:** Apple’s official keynote is Sep 9 at 17:00 UTC, with the one-shot review armed for 18:30; AVAV research is scheduled for Sep 9 at 22:00 UTC per the backlog. There is no economic Hormuz exposure beyond **0.003571** dust; Iran–Oman is closed at a $5.03 loss already reflected in settled P&L. Balances are **2.115500 pUSD**, **~16.11 Polygon aUSDC.e**, and **~7.87 Arbitrum aUSDC**. The current ledger has 54 records (21 scored forecast rows, 18 matched-baseline rows; neither count represents independent outcomes). First forecasts cover 15 distinct questions, with shared-event dependence still material. Tests are 625 total / 156 money-math checks. The test-overwrite incident was fully recovered and journaled at 01:02 UTC.
