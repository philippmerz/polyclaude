...
**Last updated:** 2026-08-28 (state snapshot updated every cron tick; this header on structural changes)
...
## Current state (snapshot 2026-08-28 16:08 UTC)
...
**Total bankroll $183.33; REALIZED +$4.66 (+2.7%) vs the $170 reference** — realized settled cash is the metric that counts (operator directive 2026-08-18: "it's only truly return when the cash settles on the wallet"). Marked unrealized is +$1.83, but executable-depth unrealized is -$4.69: the 14 PM rows (12 economic positions) are $167.16 at midpoint versus $160.63 realizable on $165.33 cost, a $6.53 thin-book overstatement. Six maker sells rest. Book: Dec-31 fades (Trump-out NO, Greenland NO), touchscreen-MacBook NO, three HLE leaderboard NO lottery legs, the matched Metamask FDV monotonicity structure (>$700M YES, >$3B/>$4B NO), the Aug-31 Iran-Oman agreement ticket, the Sep-20 United Russia 295–339 range, and DEC-0085's short-dated Google Maps rollout catalyst. DEC-0085/0086 economically closed the remaining Hormuz-normal NO at 0.997 and bought 40.95563 Lake-America-by-Aug-31 YES for $12.339349 total risk including fees, at about 0.293 average raw price under a hard 0.30 cap. Official GNIS already changed; direct US-region Maps still showed Lake Ontario at entry. Central/stressed p_yes are 0.60/0.50, deliberately sized below point-estimate Kelly because the majority-US rollout clock crosses a weekend. The Duma set remains 20 equal YES shares across 295–309 / 310–324 / 325–339 for $11.40 total and must never be traded leg-by-leg. The Iran-Oman agreement prior remains 0.09; its 31 YES announcement-lottery shares remain a no-add hold because a 0.04 taker exit returns only ~$1.24 versus $2.79 expected. Entry gates include structured per-market fee schedules, equal-share bundle atomicity, maker-price evaluation, pessimistic scenario checks, correlation-aware sizing, and criteria rotation. Money-math suite: 116 checks plus mutation harness, pre-commit enforced. Short-dated ledger N=51.
...
- `portfolio_kelly.py` — full-book Kelly audit (per-position sizing now inline in `polyclaude_enter.py`). `--constrained` reserves capital already committed to matched/set-only structures before scaling ordinary legs. Set-only legs never emit per-leg add/trim recommendations.
...
### Execution
- `polyclaude_enter.py` — mandatory unified entry helper: Gamma lookup → UMA reject → catalyst_check (or --my-p) → structured fee curve → Kelly+ρ sizing → `--execute`. Repeated `--bundle-slug` mode validates one negRisk event, equal integer shares, union-level robust EV, live CLOB fees/delay/minimums, same-event plus configured-cluster exposure, pUSD reservations/allowances, and exact on-chain settlement after every FOK. It resolves asynchronous CLOB trade IDs before accepting a match and will start only when every possible partial-fill unwind has fee-aware depth inside a bounded total-loss budget. Definite no-fills unwind observed prior fills and re-prove every token's baseline; ambiguous or delayed states halt for reconciliation instead of risking another naked leg.
