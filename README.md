...
## Current state (snapshot 2026-08-31 02:12 UTC)
...
**Total bankroll $215.79; REALIZED +$33.59 (+19.8%) vs the $170 reference** — realized settled cash is the metric that counts (operator directive 2026-08-18: "it's only truly return when the cash settles on the wallet"). The 14 PM rows cost $162.23 and mark at $168.20 (+$5.98), but executable-depth value is $158.29 (-$3.93): quote both because the thin books overstate the open sleeve by $9.91. The later Guardian Larak-strike alert was duplicate corroboration of the escalation already processed at 22:04, not a new probability event. Oman's official statement index still has no bilateral acceptance announcement; the Iran–Oman YES prior remains 0.05 and its 31 shares remain HOLD/no-add because $1.55 expected terminal value exceeds the roughly $0.62 executable exit through the Sep-1 03:59 UTC cutoff. Six maker sells and zero BUY commitments rest. The Aug-31 HLE review validated the archive instrument against known 2025 changes, passed Claude/Grok/Gemini coverage, and again found the live agi.safe.ai board identical to 2026-01-15. The next-Gemini-Pro debut NO ladder now asks 0.15/0.17/0.31/0.62 at the 40/45/50/55 thresholds. The held ≥40 leg remains the best expression at p_no 0.60 / stress 0.45, but no add clears the full entry pipeline: the correlated `ai-ships-fast-short` cap is effectively binding and the helper rejects the sub-$1 incremental size. The ticket stays 169 NO, $17.60 notional / $18.22940 fee-inclusive. The Duma distribution was also rechecked: VCIOM's live table still ends Aug-23 at 39.2% United Russia support, with no independent seat forecast, so the 0.76 union prior and Sep-1..10 add gate remain unchanged. Unused proceeds earn on 26.3501 Polygon aUSDC.e at a 2.96% live hurdle; exactly $6.000000 pUSD remains immediately deployable, inside the $5–10 operational-float band. Money-math suite: 123 checks plus a 19-case mutation harness, pre-commit enforced. Short-dated ledger N=51.
...
- `decisions.json` — append-only structured decision tracker (DEC-0001 through DEC-0104 as of this snapshot)
