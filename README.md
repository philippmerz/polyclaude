...
## Current state (snapshot 2026-09-01 18:05 UTC)
...
**Total bankroll $207.67; REALIZED +$22.04 (+13.0%) vs the $170 reference** — realized settled cash is the metric that counts. The **13 active indexed PM rows** cost $157.20 and mark at $166.85 (+$9.66), but executable-depth value is $160.09 (+$2.89): quote both because thin books overstate the active sleeve by $6.76. Data-api also retains one settled losing Iran–Oman row, now excluded from open P&L and advisory tools. It finalized NO on Sep-1; DEC-0082's 31 YES shares lost **$5.029998 gross**, correcting realized P&L immediately instead of waiting for de-indexing. `redeem-all` correctly sent no transaction: it now requires the held outcome itself to be final at $1, because data-api labels losing rows redeemable too. Hormuz-normal is de-indexed and still unproposed, but DEC-0086 had already sold 12.11 NO; the archive correctly retains only **0.003571 dust**, not a live $12 claim. Six maker sells and zero BUY commitments rest. Duma remains a 0.68-fair equal-share HOLD with its add gate failed. The wallet has 9.732500 deployable pUSD and 16.103041 Polygon aUSDC.e. GTA's 18:02 counter reversal did not clear the fee-aware robust re-entry gate. Two irrelevant post-resolution Iran alerts exposed a stale `news_watcher` process; it was restarted and verified singular on the settled-row filter, and the check-in contract now repeats freshness verification after daemon edits. Money-math suite: 123 checks plus a 19-case mutation harness, pre-commit enforced. Short-dated ledger N=51.
...
Ostium: 0 open perps (SPX / NDX / XAU all TP-closed May-2026; planned OLP deposit parked pre-exploit — zero exposure through both exploit reports, $18M→$24M revised). Crypto sleeve has ~$7.87 aUSDC Aave-Arb plus small stable/gas balances. The PM wallet holds 16.103041 aUSDC.e on Polygon, $9.732500 pUSD operational float, and small raw stable/gas balances after closing DEC-0110. Status scripts read aTokens/pUSD directly, so idle capital is always visible. Run `scripts/crypto_status.py` + `scripts/aave_deposit.py rate` for live figures.
