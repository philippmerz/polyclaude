...
## Current state (snapshot 2026-09-02 10:11 UTC)
...
**Total bankroll $208.30; REALIZED +$21.92 (+12.9%) vs the $170 reference** — realized settled cash is the metric that counts. The **14 active indexed PM rows** cost $165.96 and mark at $176.42 (+$10.46), but executable-depth value is $170.94 (+$4.98): quote both because thin books overstate the active sleeve by $5.48. Data-api also retains one settled losing Iran–Oman row, now excluded from open P&L and advisory tools. It finalized NO on Sep-1; DEC-0082's 31 YES shares lost **$5.029998 gross**. `redeem-all` correctly sent no transaction: it requires the held outcome itself to be final at $1, because data-api labels losing rows redeemable too. Hormuz-normal is de-indexed and still unproposed, but DEC-0086 had already sold 12.11 NO; the archive retains only **0.003571 dust**, not a live $12 claim. Six maker sells and zero BUY commitments rest. Duma remains a 0.68-fair equal-share HOLD: Sep-2 complete bids/asks are 0.652/0.683 and the add gate still fails. DEC-0114 now holds 12 GTA-under-20M YES shares bought at 0.73 (**$8.878260 gross including fee**): at hour 129 the official count was 17,829,395, the threshold required 1.336M/day for the final 39 hours, and every measured trailing window through 60 hours projected below 20M. The wallet retains 0.854240 pUSD and about 16.10 Polygon aUSDC.e; the bounded entry was a model-risk choice, not a capital constraint. A fresh Sep-1 macOS/OLED clue supports the touchscreen MacBook pipeline but still says the machine is due by early 2027 and gives no 2026 sale date, so p_no remains 0.65 ahead of the Sep 8–10 event. All four daemons are singular/current. Money-math suite: 123 checks plus a 19-case mutation harness, pre-commit enforced. Short-dated ledger N=51.
...
Ostium: 0 open perps (SPX / NDX / XAU all TP-closed May-2026; planned OLP deposit parked pre-exploit — zero exposure through both exploit reports, $18M→$24M revised). Crypto sleeve has ~$7.87 aUSDC Aave-Arb plus small stable/gas balances. The PM wallet holds 16.103919 aUSDC.e on Polygon, $0.854240 pUSD operational float, and small raw stable/gas balances after opening DEC-0114. Status scripts read aTokens/pUSD directly, so idle capital is always visible. Run `scripts/crypto_status.py` + `scripts/aave_deposit.py rate` for live figures.
