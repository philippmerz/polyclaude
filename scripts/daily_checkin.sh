...
BOUNDED RUN CONTRACT (operator-authorized 2026-09-08): Complete this due checklist and concrete necessary follow-up/verification once, then end. Do not create or maintain an indefinite durable goal, schedule idle self-follow-ups, or poll/wait for the next tick. Cron and event watchers handle waiting; all required safety checks remain in force. This supersedes historical instructions to keep an ROI goal active through quiet continuation turns.

RESOURCE PRE-FLIGHT: Run `./scripts/check_usage.sh --brief` before expensive discretionary research. Use the main-Codex headroom to reserve the primary context for portfolio/risk judgment and route bounded routine work to cheaper subagents. Never skip required safety checks or delay a thesis-break exit to save quota. This probe calls the read-only app-server account endpoint directly; do not inject `/usage` as a conversational turn.
...
3c. EXIT ROUTING: run `.venv/bin/python scripts/exit_analysis.py` — for every position it computes central Bernoulli hold EV (fee-free resolution: `p × $1 + (1-p) × $0` per share) vs TAKER-SELL net (walks every REAL bid-book prefix so an EV-improving partial trim is not hidden by worse lower bids, and charges each actual fill using `rate × [p × (1−p)]^exponent`; `scripts/pm_fees.py` reads the authoritative structured `feeSchedule`, with the 0.07 cap retained only for legacy compatibility) vs the MAKER breakeven (= fair, since post-only pays no fee). Act on any SELL-TAKER verdict after the stated hidden-info verification. A HOLD-CANDIDATE verdict is only a central-arithmetic screen, never a recovery thesis or complete decision: before finalizing it, explicitly compare executable exit plus redeployment with a pessimistic p case, the marginal-hurdle check, the Kelly advisory, and explicit joint terminal states for material correlated exposure. `portfolio_kelly.py` uses midpoint prices and a rho heuristic; do not call it a full joint loss-distribution model. For ordinary public-information positions, consider resting a maker sell AT fair (free option, fills only if someone pays >= fair). On hidden-info-class positions, never rest a sell at or below fair: an informed up-spike can mean fair JUMPED. A premium-to-fair sell (strictly above fair) is allowed only when the premium explicitly compensates that jump risk; thesis-break exits remain active judgments, and scheduled-catalyst pull rules still apply (doctrine in notes/resting_orders.md). Lesson sources: Prime-SDCC taker exit gave up ~\$2 vs holding (2026-07-24); Fed's maker route avoided the taker-fee breakeven gap (2026-07-28); operator correction 2026-09-21: a binary loss is terminal, so central EV must never read as expected price recovery.
...
6b. ORGANIZE DISCOVERY CONTEXT: after both discovery passes have written snapshots, run `.venv/bin/python scripts/market_context_batches.py --scan-kind primary --format markdown --batch-size 20` and `.venv/bin/python scripts/market_context_batches.py --scan-kind thin_tail --format markdown --batch-size 20`. These offline, proof-hashed batches only order the review: they make no network, model, probability, or execution decision, and `execution_ready` remains false. Exact Gamma event grouping covers only rows observed in the filtered snapshot and does not prove proposition or settlement equivalence. Review every emitted batch plus the complete raw scanner output; re-read literal criteria and re-walk live books before any trade judgment. If an ad-hoc run uses `--max-markets`, inspect `counts.truncated_triggers` and return to omitted raw rows.
7. Journal the result. Replace README.md's single "Last audited snapshot" with the current portfolio state so the GitHub front page stays fresh. Keep README concise: never append earlier updates, historical state, postmortems, or "supersedes" narration; those belong in notes/journal.md or dated research.
...
[if positions.py prints a REALIZABLE line, quote it too: "realizable $Z.ZZ (depth-walked, net of taker fees)".
...
11. Commit + push (audit diff for secrets first). BEFORE committing, rerun the daemon stale-code one-liner from step 1 if any daemon script changed during this tick; the step-1 snapshot becomes invalid the moment you edit a long-lived daemon. Restart and verify exactly one absolute-path process before handoff. Lesson source 2026-09-01: news_watcher.py was fixed at 14:00 to exclude settled rows but remained on its pre-fix process, producing two false Iran portfolio alerts by 18:00.
...
POLYMARKET WRITE PATH (updated 2026-09-10): the v1 SDKs remain broken against v2 (`order_version_mismatch`). Route every NEW BUY through `scripts/polyclaude_enter.py`; it creates the exact exposure reservation that `clob_v2.py buy` atomically claims once, then enforces the robust-edge, 15% ticket and 30% event/configured-cluster caps under the shared entry lock. Never bypass it with a raw BUY. Direct `clob_v2.py sell` and `cancel` remain the close/order-management paths; cancel shares the lock, requires the exact target in the fully paginated pre-cancel inventory, verifies removal, and marks any matching BUY reservation for affirmative terminal/trade/`totalBought` reconciliation. Any unreserved or reappeared-cancel ambiguity persists a blocker for all new entries. CTF token IDs are unchanged across v1→v2, so existing positions can close through v2 SELL or resolve naturally. New-entry/fill collateral is pUSD on Polygon. Use `scripts/wrap_pusd.py wrap` for USDC.e → pUSD and `unwrap` for pUSD → USDC.e (or native USDC when unpaused); both routes use the deployed v2 ramps, validate the live contract and simulate before broadcast. Pull USDC from Aave → bridge via Across → wrap. Schema/addresses/32-bit salt: `research/_polymarket_v2_schema_2026-05-03.md`.
