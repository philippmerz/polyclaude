...
3c. EXIT ROUTING: run `.venv/bin/python scripts/exit_analysis.py` — for every position it computes HOLD EV (fee-free resolution) vs TAKER-SELL net (walks the REAL bid book and charges each actual fill using `rate × [p × (1−p)]^exponent`; `scripts/pm_fees.py` reads the authoritative structured `feeSchedule`, with the 0.07 cap retained only for legacy compatibility) vs the MAKER breakeven (= fair, since post-only pays no fee). Act on any SELL-TAKER verdict; for HOLD verdicts on ordinary public-information positions, consider resting a maker sell AT fair (free option, fills only if someone pays >= fair). On hidden-info-class positions, never rest a sell at or below fair: an informed up-spike can mean fair JUMPED. A premium-to-fair sell (strictly above fair) is allowed only when the premium explicitly compensates that jump risk; thesis-break exits remain active judgments, and scheduled-catalyst pull rules still apply (doctrine in notes/resting_orders.md). Lesson sources: Prime-SDCC taker exit gave up ~\$2 vs holding (2026-07-24); Fed's maker route avoided the taker-fee breakeven gap (2026-07-28).
