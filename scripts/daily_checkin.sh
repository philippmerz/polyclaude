...
POLYMARKET WRITE PATH (updated 2026-08-29): the v1 SDKs remain broken against v2 (`order_version_mismatch`). Route every NEW BUY through `scripts/polyclaude_enter.py`; it creates the exact exposure reservation that `clob_v2.py buy` atomically claims once, then enforces the robust-edge, 15% ticket and 30% event/configured-cluster caps under the shared entry lock. Never bypass it with a raw BUY. Direct `clob_v2.py sell` and `cancel` remain the close/order-management paths; cancel shares the lock, requires the exact target in the fully paginated pre-cancel inventory, verifies removal, and marks any matching BUY reservation for affirmative terminal/trade/`totalBought` reconciliation. Any unreserved or reappeared-cancel ambiguity persists a blocker for all new entries. CTF token IDs are unchanged across v1→v2, so existing positions can close through v2 SELL or resolve naturally. New-entry/fill collateral is pUSD on Polygon. To wrap more USDC.e → pUSD: approve CollateralOnramp `0x93070a847efEf7F70739046A929D47a521F5B8ee`, then call `wrap(USDC.e, eoa_address, amount_6dec)`. Pull USDC from Aave → bridge via Across → wrap. Schema/addresses/32-bit salt: `research/_polymarket_v2_schema_2026-05-03.md`.
