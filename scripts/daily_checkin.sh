...
POLYMARKET WRITE PATH (as of 2026-05-04): the v1 SDKs (py-clob-client, TS clob-client) are still broken against v2 — order_version_mismatch. Use `scripts/clob_v2.py` for ALL new entries / closes / cancels. It's a direct REST + EIP-712 signer, no SDK dependency, verified end-to-end. CLI: `clob_v2.py buy/sell/cancel/orders/orderbook`. Existing 9 v1 positions remain on the v1 stack — they resolve naturally and pay USDC.e on redemption; do NOT try to close them via clob_v2 (different exchange contract). Liquidity for new v2 entries lives in pUSD on Polygon (5 pUSD currently). To wrap more USDC.e → pUSD: approve USDC.e to CollateralOnramp 0x93070a847efEf7F70739046A929D47a521F5B8ee, then call onramp.wrap(USDC.e, eoa_address, amount_6dec). Pull USDC from Aave (Arb or Base) → bridge via Across → wrap. Schema details in research/_polymarket_v2_schema_2026-05-03.md.
